...
# --- Process only the missing transcripts (that failed or were deleted during troubleshooting) ----------------------
printf "+++ Processing the %s missing transcripts +++\n" `cat ${output_dir}/missing | wc | awk '{print $1}'`
for tat in `cat ${output_dir}/missing`; do
    echo "Processing transcript ${tat}..."
    python -u ./speech_graph.py ${tat} \
    > ${output_dir}/SpeechGraph_`zeropad ${tat} 4`_`date +%F`.txt 2>&1 # (pipe output and error msgs to text file)
done
