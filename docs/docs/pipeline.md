...
To construct speech networks that capture semantic content we created **netts**, a network algorithm that builds on state-of-the-art Natural Language Processing libraries.
Netts takes as input transcripts of spoken text (e.g. <em>I see a man</em>) and outputs the semantic speech network (Figure \ref{fig:ExampleGraph}).
In these semantic speech networks, nodes are entities (e.g. <em>I</em>, <em>man</em>).
Edges are relations between nodes (e.g. <em>see</em>).
...
Netts is fast ($\sim$ 40 seconds processing time per speech excerpt) and is robust against artefacts typical for transcribed speech, lending itself to the automated construction of speech networks from large datasets.
...
## Preprocessing
Netspy starts by expanding the most common English contractions (e.g. expanding <em>I'm</em> to <em>I am</em>), removing interjections (<em>Mh</em>, <em>Uhm</em>) and removing any transcription notes (e.g. timestamps, <em>[inaudible]</em>) that were inserted by the transcriber.
No stop words or punctuation are removed to stay as close to the original speech as possible.
This and other processing steps are customizable in netspy, e.g. with the option to pass a user-defined text file of transcription notes that should be ignored by netspy.

Netspy uses CoreNLP to perform sentence splitting, tokenization, part of speech tagging, lemmatization, dependency parsing and co-referencing on the transcript \cite{Manning2015} with the default language model implemented in CoreNLP.
...
Netspy then submits each sentence to OpenIE5 for relation extraction \cite{Mausam2012a}, where semantic relationships between entities are extracted from the sentence.
...
Next, netspy uses the previously identified part of speech tags and dependency structure  to extract edges defined by adjectives or prepositions: For instance, <em>a man on the picture</em> contains a preposition edge where the entity <em>a man</em> and <em>the picture</em> are linked by an edge labelled <em>on</em>.
...
After creation of the edge list, netspy uses the previously identified co-referencing structure to merge nodes that refer to the same entity.
...
Finally, netspy constructs a semantic speech network from the edge list using networkx and the network is plotted and the output consisting of the networkx object, the network image and the tool output messages saved \cite{networkx}.
